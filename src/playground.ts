...
*/
  ...
  d3.select("#data-nn-robust-button").on("click", () => {
    ...
    let radius = -1;
    if (state.proximity_radius < 0) {
      radius = 2 * Math.sqrt(2);
    } else {
      radius = state.proximity_radius;
    }
      ...
      if (binPlusOne > 0 || binMinusOne > 0) {
        ...
        if (binPlusOne > binMinusOne) {
          ...
          if (testData[i].label == -1) {
            ...
            flip_result += '&nbsp;i=' + i + ', pts[' + testData[i].x + ', ' + testData[i].y + '], Freq(Blue)='
                + binPlusOne + ', Freq(Orange)=' + binMinusOne + ', flipped from Orange to Blue' + '<BR>';
            num_flipped_NtoP += 1;
            testData[i].label = 1;
          }
        } else {
          ...
          if (testData[i].label == 1) {
            console.log('Robustness change: i=', i, ' pts[', testData[i].x, ', ', testData[i].y, '] flipped label to -1 ');
            flip_result += '&nbsp;i=' + i + ', pts[' + testData[i].x + ', ' + testData[i].y + '], Freq(Blue)='
                + binPlusOne + ', Freq(Orange)=' + binMinusOne + ', flipped from Blue to Orange' + '<BR>';
            num_flipped_PtoN += 1;
            testData[i].label = -1;
          }
        }
    ...
    //updateUI();
    let mse_result: string;
    mse_result = '&nbsp;Robust to Backdoor in a neighborhood of Radius=' + radius + '<BR>';
    mse_result += flip_result;
  ...
  d3.select("#data-nn-proximity-button").on("click", () => {
    ...
    // compute pair-wise distances for TRAINING DATA!!!!!
    // return the count of pair-wise distances for N, P, and NtoP labeled points
    let dist_count = proximityDist.getDataProximityDistance(trainData, maxRadius, deltaRadius);
    ...
    // print the histograms and create histogram visualization
    let text = 'Histogram bins for proximity of N (Orange) train points based on Euclidean distances';
    let histTrainN = new AppendingHistogramChart(proximityDist.getMapGlobal_Proximitydist_N(deltaRadius), text);
    let title = 'Train data: histogram of pair-wise distances between N (Orange) points ';
    let proximity_result: string = '&nbsp; TRAIN data: Num of Pairs of N (Orange) points: ' + dist_count.index_N + ' <BR>' + histTrainN.showKLHistogram('histDivTrainN', title);

    text = 'Histogram bins for proximity of P (Blue) train points based on Euclidean distances';
    let histTrainP = new AppendingHistogramChart(proximityDist.getMapGlobal_Proximitydist_P(deltaRadius), text);
    title = 'Train data: histogram of pair-wise distances between P (Blue) points';
    proximity_result += '&nbsp; TRAIN data: Num of Pairs of P (Blue) points: ' + dist_count.index_P + ' <BR>' + histTrainP.showKLHistogram('histDivTrainP', title);

    text = 'Histogram bins for proximity of N (Orange) to P (Blue) train points based on Euclidean distances';
    let histTrainNtoP = new AppendingHistogramChart(proximityDist.getMapGlobal_Proximitydist_NtoP(deltaRadius), text);
    title = 'Train data: histogram of pair-wise distances between N (Orange) and P (Blue) points';
    proximity_result += '&nbsp; TRAIN data: Num of Pairs of N to P points: ' + dist_count.index_NtoP + ' <BR>' + histTrainNtoP.showKLHistogram('histDivTestP', title);

    let tmp = 0;
    let optimal_objective = -1;
    let optimal_bin = -1;
    state.proximity_radius = -1;
    for (let i = 0; i < hist_proximity_N.length; i++) {
      //console.log('i=', i, ' hist_proximity_N[i]=', hist_proximity_N[i], ' hist_proximity_P[i]=', hist_proximity_P[i], ' hist_proximity_NtoP[i]=', hist_proximity_NtoP[i]);
      if (hist_proximity_NtoP[i] > 0) {
        tmp = (hist_proximity_N[i] * hist_proximity_P[i]) / hist_proximity_NtoP[i];
        if (tmp > optimal_objective) {
          optimal_objective = tmp;
          optimal_bin = i;
        }
      }
    }
    if (optimal_bin >= 0) {
      proximity_result += '&nbsp; optimal radius =[' + (optimal_bin * deltaRadius) + ',' + (optimal_bin * deltaRadius + deltaRadius) + '];  Max objective =' + optimal_objective + ' <BR>';
      state.proximity_radius = (optimal_bin * deltaRadius + deltaRadius * 0.5);
    } else {
      proximity_result += '&nbsp; Error while looking for optimal radius <BR>';
    }
...
/** Added: change to export function */
...
function oneStep(): void {
  ...
  trainData.forEach((point, i) => {
    // debug PB
    //console.log("DEBUG point i=", i, "point[", point.x, ", ", point.y, "]");
    ...
    if ((i + 1) % state.batchSize === 0) {
      ...
      //   for (let k = 0; k < network[j].length; k++){
      //     // if (isNaN(network[j][k].bias)){
      //     // console.log("INFO after updateWeights :network[", j, "][", k, "].inputDer=", network[j][k].inputDer);
      //     // console.log("INFO after updateWeights:network[", j, "][", k, "].bias=", network[j][k].bias);
      //     // }
      //   }
      // }
    }
...
/** Added end of ste and get weights and biases */
//////////////////////////////////////////////
function reset(onStartup=false) {
  ...
  // PB TODO decide whether nn-Activation.LINEAR should be the default for all problem types !!
...
function generateData(firstTime = false) {
  ...
  /* Added: end of generator extension */
  //////////////////////////////////////////////////
  ...
  /** Added: added to support the data calculator */
